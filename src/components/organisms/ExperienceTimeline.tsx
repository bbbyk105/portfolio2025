  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {experiences.map((exp, index) => (
        <div key={exp.id} className="mb-12 relative flex flex-col md:flex-row">
          {/* Timeline dot with zoom-in animation */}
          <div
            data-aos="zoom-in"
            data-aos-delay={300 + index * 100}
            className="absolute left-[-8px] md:left-1/2 md:transform md:translate-x-[-50%] w-4 h-4 rounded-full bg-teal-400 border-4 border-gray-900 z-10"
          ></div>

          {/* Left side content (odd items) or spacer (even items) */}
          <div className="md:w-1/2 md:pr-12">
            {index % 2 === 0 ? (
              ...
              <div
                ...
                className="bg-gray-900 p-6 rounded-lg shadow-lg transform transition-all duration-300 hover:shadow-teal-900/20 hover:-translate-y-1 ml-6 md:ml-0"
              >
          ...
          {/* Right side content (even items) or spacer (odd items) */}
